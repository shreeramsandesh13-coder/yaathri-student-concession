import React from 'react';
import { Link } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import RtoPortal from '../../components/RtoPortal';
import Button from '../../components/ui/Button';

/**
 * RTO PORTAL PAGE (/rto/*)
 * State Motor Vehicles Department supervisory console, verifier licensing, and audit stream.
 */
export default function RtoPortalPage() {
  const { isRto, isAuthenticated } = useAuth();

  if (!isAuthenticated || !isRto) {
    return (
      <div className="max-w-md mx-auto text-center py-20 px-6 bg-white dark:bg-[#0B111D] border border-slate-200 dark:border-slate-800 rounded-3xl shadow-xl my-12 space-y-6">
        <div className="w-16 h-16 rounded-full bg-indigo-500/10 text-indigo-500 mx-auto flex items-center justify-center">
          <span className="material-symbols-outlined text-3xl">policy</span>
        </div>
        <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
          RTO Directorate Clearance Required
        </h3>
        <p className="text-slate-600 dark:text-slate-400 text-xs sm:text-sm leading-relaxed">
          Please sign in with authorized Kerala Motor Vehicles Department supervisory credentials.
        </p>
        <Link to="/login" className="block">
          <Button variant="primary" size="md" className="w-full">
            Sign In to RTO Authority &rarr;
          </Button>
        </Link>
      </div>
    );
  }

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-6">
      <RtoPortal />
    </div>
  );
}

